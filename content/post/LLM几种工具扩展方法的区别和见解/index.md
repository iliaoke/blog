...
- **LLM 本质上是一个“语言决策器/规划器（Planner）”**：它接收上下文与指令，产出下一步该说什么、该做什么。
- **真正的执行（Executor）发生在模型之外**：例如 HTTP 请求、数据库查询、运行脚本、读写文件、调用内部服务等。
...
- **连接与调用（桥接层）**：`Tool Calls` 通常需要以代码形式实现调用链路与编排逻辑，包括触发判定、参数提取与校验、失败重试、超时控制、结果回传等；`Skills` 则倾向于允许以自然语言描述触发条件与执行规则，将部分连接、调用与判断下沉至宿主环境与 AI 侧处理。
